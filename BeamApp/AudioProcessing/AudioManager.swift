...
final class AudioManager: ObservableObject {
    ...
    func playLocalTrack(from fileURL: URL) {
    ...
    private func checkForTrackCompletion() {
        if let avPlayer = avPlayer, let currentItem = avPlayer.currentItem {
            let currentPlaybackTime = CMTimeGetSeconds(currentItem.currentTime())
            let totalDuration = CMTimeGetSeconds(currentItem.duration)
            
            if (currentPlaybackTime < lastCheckedTime) || (totalDuration > 0 && (totalDuration - currentPlaybackTime) <= 1) {
                NotificationCenter.default.post(
                    name: AudioManager.audioDidFinishNotification,
                    object: nil
                )
    ...
    private func updatePlaybackTime() {
        if let avPlayer = avPlayer {
            ...
            duration = CMTimeGetSeconds(avPlayer.currentItem?.duration ?? CMTime.zero)
        } else {
            currentTime = musicPlayerController.currentPlaybackTime
            duration = musicPlayerController.nowPlayingItem?.playbackDuration ?? 0
        }
